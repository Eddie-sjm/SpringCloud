    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="changSaleable(props.item.id)">下架</v-btn>
          <v-btn icon v-else @click="changSaleable(props.item.id)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteGoods(id){
        this.$http.delete("/item/spu/delete/"+id)
          .then(() => {
            // 删除成功，重新加载数据
      ...
      changSaleable(id){
        this.$message.confirm(this.filter.saleable === 0 ? "请到上架或下架执行操作！":this.filter.saleable ? "确认要下架?":"确认要上架?")
          .then(() => {
            if(this.filter.saleable === 0){
              // this.$message.success("请到上架或下架执行操作！");
            }else {
              this.$http.put("/item/spu/" + id)
                .then(() => {
